...
with tab2:
    ...
    if st.button("Train Model", use_container_width=True):  # Adding a button to trigger model training
        with st.spinner("Training model..."):  # Displaying a status message while training the model
            ...
            ingest_data(st.session_state.master_data_path, st.session_state.mongodb_host, st.session_state.mongodb_port, st.session_state.mongodb_db,"Twitter_training")  # Calling the ingest_data function
